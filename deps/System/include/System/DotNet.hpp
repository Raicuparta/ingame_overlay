...
class DotNetCoreHost
{
  ...
  public:
    ...
    DefaultDelegateType LoadAssemblyAndEntryPoint(
        ...
        std::string const &assembly_runtimeconfig = std::string());
    ...
    /// <returns>entry point delegate</returns>
    template <typename T>
    inline typename FunctionTraits<T>::F
    GetFunctionDelegate(std::string const &full_qualified_class_name, std::string const &static_public_function_name, std::string const &full_qualifed_delegate_type)
    {
